...
from trainingUtils.utils import evaluate
from dataloader.czech_slr_dataset import CzechSLRDataset
from models.spoter_model_original import SPOTER, SPOTERnoPE
from models.BaselineTransformerClassification import BaselineTransformerClassification
from models.ExplainabTransformer import ExplainabTransformerwQuery, ExplainabTransformerwSequence
...
def test(args):
    ...
    #Load weigths:
    slrt_model.load_state_dict(torch.load(args.load_checkpoint, map_location=torch.device('cpu')).state_dict())
    slrt_model.eval()
